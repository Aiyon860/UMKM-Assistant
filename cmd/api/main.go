...
func main() {
	...
	mockCaptionSvc := service.NewMockCaptionService()
	captionHandler := handler.NewCaptionHandler(mockCaptionSvc)
	...
	r.HandleFunc("POST /api/v1/caption-generator", http.HandlerFunc(captionHandler.GenerateCaption))
	r.Handle("POST /api/v1/chatbot", http.HandlerFunc(chatbotHandler.Chat))
